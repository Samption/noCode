package com.sxl.nocode.ratelimit.aspect;
...
public class RateLimitAspect {
    ...
    /**
     * 生成限流器的key，根据限流类型（API、USER、IP）生成不同的key，实现不同的限流策略
     * 1. API：接口级别，根据方法名生成key，相同的接口方法名共享一个key，实现接口级别的限流
     * 2. USER：用户级别，根据用户ID生成key，相同的用户ID共享一个key，实现用户级别的限流
     * 3. IP：IP级别，根据IP地址生成key，相同的IP地址共享一个key，实现IP级别的限流
     * @param point
     * @param rateLimit
     * @return
     */
